...
vec2 flow = curl(uvMid * 2.0 + uTime * 0.15);
...
float r = length(uvMid);

float angle = atan(uvMid.y, uvMid.x);

angle += r * 2.0 + uTime * uSpeed * 3.5;

uvMid = vec2(cos(angle), sin(angle)) * r;
...
vec2 samplePos = uvMid + lightDir * stepSize;
...
vec3 shift1 = vec3(0.2,0.3,0.6);
vec3 shift2 = vec3(0.3,0.6,0.8);
vec3 shift3 = vec3(0.4,0.2,0.7);
...
// ───── Chromatic Nebula Scattering ─────

float scatter = smoothstep(0.3,0.9,n);

vec3 chromaA = vec3(0.2,0.45,0.9);   // cyan
vec3 chromaB = vec3(0.6,0.25,0.8);   // violet

vec3 chroma = mix(chromaA,chromaB,n);

col += chroma * scatter * 0.18;
