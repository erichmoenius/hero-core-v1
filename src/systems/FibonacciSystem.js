...
export class FibonacciSystem {
  constructor(scene) {
    ...
    this.cyanLight = new THREE.PointLight(0x66ccff, 12.0, 50);

    this.cyanLight.position.set(-4, 2, 3);

    this.scene.add(this.cyanLight);

    this.violetLight = new THREE.PointLight(0x8866ff, 10.0, 50);

    this.violetLight.position.set(4, -2, 2);

    this.scene.add(this.violetLight);

    this.goldLight = new THREE.PointLight(0xffd38a, 4.0, 50);

    this.goldLight.position.set(0, 4, -2);
  ...
  createParticles() {
    ...
    for (let i = 0; i < this.N; i++) {
      ...
      const mat = new THREE.MeshStandardMaterial({
        color: new THREE.Color(0.42, 0.43, 0.46),

        emissive: new THREE.Color(0.0015, 0.002, 0.03),
      ...
      mesh.userData.roughness = THREE.MathUtils.randFloat(0.12, 0.3);

      mesh.userData.metalness = THREE.MathUtils.randFloat(0.88, 1.0);
      ...
      mesh.userData = {
        ...
        brightness: 0.88 + Math.random() * 0.18,

        roughness: 0.18 + Math.random() * 0.12,

        metalness: 0.88 + Math.random() * 0.08,
  ...
  update(delta = 0.016, audio = null) {
    ...
    this.rimLight.position.x = -6 + Math.sin(this.time * 0.08) * 2;

    this.rimLight.position.y = 3 + Math.cos(this.time * 0.07) * 1;

    this.warmLight.position.x = 6 + Math.cos(this.time * 0.14) * 1.5;

    this.warmLight.position.y = -2 + Math.sin(this.time * 0.12) * 0.8;
    ...
    if (this.morphProgress < 1) {
      ...
      this.meshes.forEach((m) => {
        m.position.lerpVectors(m.userData.from, m.userData.to, ease);
      });
    } else {
    ...
    const s = 1 + Math.sin(this.time * 1.2) * 0.03 + energy * 0.05;
    ...
    this.meshes.forEach((m, i) => {
      ...
      const shimmer = Math.sin(this.time * 1.5 + i * 0.25) * 0.35;

      const patina = Math.sin(i * 2.37) * 0.5 + 0.5;

      const ageTint = m.userData.age * 0.025;

      const base = 0.34;

      m.material.color.setRGB(
        base * m.userData.brightness,

        (base + m.userData.patina * 0.012) * m.userData.brightness,

        (base + 0.03 - m.userData.patina * 0.008) * m.userData.brightness,
      );

      m.material.roughness = m.userData.roughness;

      m.material.metalness = m.userData.metalness;

      const sparkle = Math.sin(this.time * 2.2 + i * 0.73);

      if (sparkle > 0.992 && m.userData.scratch > 0.9) {
      ...
      const spectral = Math.sin(this.time * 0.8 + i * 0.11);

      if (spectral > 0.6) {
        m.material.emissive.setRGB(0.03, 0.08, 0.12);
      } else if (spectral < -0.6) {
        m.material.emissive.setRGB(0.1, 0.07, 0.02);
      } else {
        m.material.emissive.setRGB(0.03, 0.03, 0.05);
      }
    });

    // ---------------- DEPTH DRIFT ----------------

    this.group.position.z = -2 + Math.sin(this.time * 0.5) * 0.3;
  }
