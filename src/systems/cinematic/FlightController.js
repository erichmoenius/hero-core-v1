...
export class FlightController {
  ...
  update(delta = 0.016, audio = {}) {
    ...
    const floatY =
      Math.sin(this.time * this.floatSpeed) *
      (this.floatStrength + energy * 0.08);
    ...
    const force = desired.clone().sub(this.camera.position);

    this.velocity.add(force.multiplyScalar(this.positionDamping));
    ...
    const lookForce = this.lookTarget.clone().sub(currentLook);

    this.lookVelocity.add(lookForce.multiplyScalar(this.lookDamping));

    this.lookVelocity.multiplyScalar(0.9);

    currentLook.add(this.lookVelocity);
